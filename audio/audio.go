...
var (
        ...
        keybindObj *keybinding.MediaKey
)
...
func updateSourceOutput(_index C.int,
        ...
        case C.PA_SUBSCRIPTION_EVENT_CHANGE:
                if audio.sourceOutputs[index] != nil {
                        ...
                        changes := getDiffProperty(newso, audio.sourceOutputs[index])
                        audio.sourceOutputs[index] = newso
                        //audio.sourceOutputs[index].InputLevel = float64(inputlevel)
                        //dbus.NotifyChange(audio.sourceOutputs[index], "InputLevel")
                        for key, _ := range changes {
                                dbus.NotifyChange(audio.sourceOutputs[index], key)
                        }
                }
        ...
        case C.PA_SUBSCRIPTION_EVENT_REMOVE:
                ...
                fmt.Println("source output deleted")
                break
        }
}

//export updateSourceOutputInputLevel
func updateSourceOutputInputLevel(_index C.int,
        inputlevel C.float) {
        index := int(_index)
        if audio.sourceOutputs[index] != nil {
                audio.sourceOutputs[index].InputLevel = float64(inputlevel)
...
func (audio *Audio) listenMediaKey() {
        keybindObj.ConnectAudioMute(func(pressed bool) {
                if !pressed {
                        sink := audio.GetDefaultSink()
                        fmt.Println("Toggle mute mode: ", !sink.Mute)
                        sink.SetSinkMute(!sink.Mute)
                        if apiSound, err := libsound.NewSound(API_SOUND_DEST,
                                API_SOUND_PATH); err != nil {
                                Logger.Warning("New Sound For Play Failed: ", err)
                        } else if apiSound != nil {
                                apiSound.PlaySystemSound("audio-volume-change")
                        }
                }
        })
        keybindObj.ConnectAudioUp(func(pressed bool) {
                if !pressed {
                        sink := audio.GetDefaultSink()
        ...
        keybindObj.ConnectAudioDown(func(pressed bool) {
                if !pressed {
                        sink := audio.GetDefaultSink()
                        volume := int32(sink.Volume - _VOLUME_STEP)
                        if volume < 0 {
...
func (audio *Audio) getSourceOutputs() []*SourceOutput {
        ...
        for i := 0; i < n; i = i + 1 {
                so := getSourceOutputFromC(audio.pa.source_outputs[i])
                audio.sourceOutputs[int(so.Index)] = so

        }
        sourceOutputs := make([]*SourceOutput,
                len(audio.sourceOutputs))
        var j int
...
func (sink *Sink) OnPropertiesChanged(name string, oldv interface{}) {
        ...
        fmt.Printf("Properties changed,%v changed: %v\n", name, oldv)
        switch name {
        case "Mute":
                if oldv == sink.Mute {
                        break
...
func (source *Source) getSourceOutputs() []*SourceOutput {
        ...
        audio.lock.Unlock()
        fmt.Println("sync lock unlocked")
        ...
        for _, value := range all {
                if value.Source == source.Index {
                        sourceOutputs[j] = value
                        j = j + 1
                        audio.sourceOutputs[int(value.Index)] = value
                }
        }
...
func (source *Source) CreateStream() {
...
func main() {
        ...
        if keybindObj, err = keybinding.NewMediaKey(MEDIA_KEY_DEST,
                MEDIA_KEY_PATH); err != nil {
                Logger.Fatal(err)
        }
