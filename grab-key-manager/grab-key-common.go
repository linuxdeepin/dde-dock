...
func GetXGBShortcut(shortcut string) string {
	...
	for i, v := range array {
		...
		if v == "primary" && !strings.Contains(value, "control") {
			value += "control"
		} else if v == "alt" || v == "super" ||
			v == "meta" || v == "num_lock" ||
			v == "caps_lock" || v == "hyper" {
...
func NewKeyInfo(state uint16, keycode xproto.Keycode) *_KeyInfo {
	return &_KeyInfo{State: state, Detail: keycode}
}

func GetExecAction(k1 *_KeyInfo) (bool, string) {
	for k, v := range _KeyBindings {
