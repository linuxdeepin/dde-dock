...
void deselect_events(Display * display)
{
	XIEventMask m;
	m.deviceid = XIAllMasterDevices;
	m.mask_len = 0;
...
void listen(Display * display)
{
	...
	int root_x, root_y, nouse, go_handle_ret;
	Window noused_window;
	unsigned int mask;
	while (1) {
		XEvent ev;
		XGenericEventCookie *cookie =
		    (XGenericEventCookie *) & ev.xcookie;
			...
			case XI_RawTouchEnd:
				{
					XQueryPointer(display, root,
						      &noused_window,
						      &noused_window, &root_x,
						      &root_y, &nouse, &nouse,
						      &mask);
					XIRawEvent *event = cookie->data;
					go_handle_ret =
					    go_handle_raw_event(cookie->evtype,
								event->detail,
								root_x, root_y,
								mask);
				}
...
int init_display()
{
	...
	int event, error;
	if (!XQueryExtension
	    (display, "XInputExtension", &xi2_opcode, &event, &error)) {
